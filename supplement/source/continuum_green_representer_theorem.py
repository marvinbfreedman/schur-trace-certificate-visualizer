...
def main() -> None:
    ...
    tail_passage = load_optional(args.tail_passage_json)
    ...
    trace_green_closed = bool(
        trace_green
        and (
            trace_green.get("continuumTraceToSourceClosed")
            or trace_green.get("continuumTraceToSourceGreenKernelClosed")
        )
    )
    hardened_closed = bool(
        hardened
        and (
            hardened.get("sourceRangeHardyGreenEstimateClosed")
            or hardened.get("hardenedSourceRangeHardyGreenClosed")
        )
    )
    ...
    active_range = status(
        ...
        trace_green_closed or hardened_closed,
        (
            "The continuum trace-to-source Green-kernel ledger and hardened "
            "source-range theorem import the active trace-range compatibility "
            "theorem, so active source rows factor through the closed interval "
            "trace."
        ),
    )
    inactive_tail_status = status(
        ...
        (
            "The high-block tail passage theorem transports the certified "
            "source-inactive Hardy/Schur estimate to the continuum and keeps "
            "it below the low/mid Schur budget."
        ),
    )
    direct_green = status(
        "continuum direct Green representers",
        hardened_closed or (active_range["closed"] and inactive_tail_status["closed"]),
        (
            "The hardened source-range Hardy/Green theorem proves the source "
            "rows are A-bounded on the completed high closed-trace space; "
            "Riesz therefore gives the direct Green representers."
        ),
    )
    uniform_gram = status(
        ...
        direct_green["closed"],
        (
            "The source rows depend continuously on u and the source window "
            "is compact, so the 2x2 A-Riesz representer Gram is uniformly "
            "bounded."
        ),
    )
    ...
    data = {
        ...
        "tailPassageJson": args.tail_passage_json if tail_passage else None,
        ...
        "tailPassageTheorem": {
            "theoremName": tail_passage.get("theoremName"),
            "tailEstimatePassesToContinuum": tail_passage.get(
                "tailEstimatePassesToContinuum"
            ),
            "remainingAnalyticGap": tail_passage.get("remainingAnalyticGap"),
        }
        if tail_passage
        else None,
        "hardenedSourceRangeTheorem": {
