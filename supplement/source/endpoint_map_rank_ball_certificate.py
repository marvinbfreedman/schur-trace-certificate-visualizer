...
def main() -> None:
    ...
    parser.add_argument(
        "--endpoint-interval-theorem-json",
    ...
    endpoint_interval = load_optional(args.endpoint_interval_theorem_json)
    if "endpointMap" not in data:
        raise SystemExit(
    ...
    interval_endpoint_closed = bool(
        endpoint_interval
        and (
            endpoint_interval.get("unconditionalContinuumEndpointClosed")
            or endpoint_interval.get("theoremClosed")
        )
    )
    ...
    interval_status = status(
        "interval endpoint theorem supersedes diagnostic ball route",
        interval_endpoint_closed,
        (
            "The active endpoint map full-rank proof is supplied by the "
            "adjoint Green endpoint range interval theorem.  This rank-ball "
            "record remains a diagnostic/conditional perturbation check, not "
    ...
    data_out = {
        ...
        "endpointIntervalTheoremJson": args.endpoint_interval_theorem_json
        ...
        "endpointIntervalTheoremSupersedesDiagnosticStatus": interval_status,
        "proof": [
        ...
        if rigorous_full_rank or interval_endpoint_closed
        else (
