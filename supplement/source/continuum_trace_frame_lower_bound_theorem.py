...
def first_float(*values, default=None):
    for value in values:
...
def main() -> None:
    ...
    frame_drift = (
        first_float(
            quadrature.get("metricRelativeTraceQuadratureErrorBound"),
            quadrature.get("maxAbsFrameMinRelativeDrift"),
            default=0.0,
        )
        if quadrature
        else None
    ...
    density_drift = (
        first_float(quadrature.get("maxAbsMaxRowDensityL2RelativeDrift"), default=0.0)
        if quadrature
        else None
    )
