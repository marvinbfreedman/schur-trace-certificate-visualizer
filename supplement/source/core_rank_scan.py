...
try:
    ...
    raise SystemExit("core_rank_scan.py requires numpy; run with PYTHONPATH=/private/tmp/codex_numpy") from exc
...
def main():
    ...
    parser.add_argument("--core-nmax", type=int, default=3)
    ...
    for i, x in enumerate(xs):
        for j, y in enumerate(xs[: i + 1]):
            value = full_partial_h(float(x), float(y), args.omega, args.core_nmax, rs, ws)
            mat[i, j] = mat[j, i] = value
    evals = np.linalg.eigvalsh(mat)
    max_eval = max(evals[-1], 1.0)
    print(f"omega={args.omega:g} n={args.n_grid} x=[{-args.xmax:g},{args.xmax:g}] core<={args.core_nmax}")
